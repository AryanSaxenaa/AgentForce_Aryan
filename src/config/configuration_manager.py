...
class TestGeneratorConfig:
    ...
    default_test_framework: Dict[str, str] = None
    ...
    def __post_init__(self):
        if self.default_test_framework is None:
...
class ConfigurationManager(IConfigurationManager):
    ...
    def _load_from_env(self) -> None:
        ...
        if os.getenv('AI_PROVIDER'):
            self.config.ai_provider = os.getenv('AI_PROVIDER')
        if os.getenv('OPENAI_MODEL'):
            self.config.openai_model = os.getenv('OPENAI_MODEL')
        if os.getenv('ANTHROPIC_MODEL'):
            self.config.anthropic_model = os.getenv('ANTHROPIC_MODEL')
        
        # Numeric settings with validation
        try:
            if os.getenv('AI_MAX_TOKENS'):
                self.config.ai_max_tokens = int(os.getenv('AI_MAX_TOKENS'))
            if os.getenv('AI_TEMPERATURE'):
                self.config.ai_temperature = float(os.getenv('AI_TEMPERATURE'))
            if os.getenv('AI_TIMEOUT'):
                self.config.ai_timeout = int(os.getenv('AI_TIMEOUT'))
            if os.getenv('COVERAGE_THRESHOLD'):
                self.config.coverage_threshold = float(os.getenv('COVERAGE_THRESHOLD'))
            if os.getenv('MAX_TEST_CASES_PER_FUNCTION'):
                self.config.max_test_cases_per_function = int(os.getenv('MAX_TEST_CASES_PER_FUNCTION'))
        except ValueError:
            pass  # Keep defaults if invalid values
        
        # Boolean settings
        if os.getenv('INCLUDE_EDGE_CASES'):
            self.config.include_edge_cases = os.getenv('INCLUDE_EDGE_CASES').lower() in ('true', '1', 'yes')
        if os.getenv('INCLUDE_INTEGRATION_TESTS'):
            self.config.include_integration_tests = os.getenv('INCLUDE_INTEGRATION_TESTS').lower() in ('true', '1', 'yes')
        if os.getenv('GITHUB_INTEGRATION'):
            self.config.github_integration = os.getenv('GITHUB_INTEGRATION').lower() in ('true', '1', 'yes')
        if os.getenv('AUTO_PR_COMMENTS'):
            self.config.auto_pr_comments = os.getenv('AUTO_PR_COMMENTS').lower() in ('true', '1', 'yes')
        if os.getenv('COVERAGE_CHECK_ENABLED'):
            self.config.coverage_check_enabled = os.getenv('COVERAGE_CHECK_ENABLED').lower() in ('true', '1', 'yes')
        
        # String settings
        if os.getenv('OUTPUT_FORMAT'):
            self.config.output_format = os.getenv('OUTPUT_FORMAT')
        if os.getenv('TEST_FILE_PREFIX'):
            self.config.test_file_prefix = os.getenv('TEST_FILE_PREFIX')
        if os.getenv('TEST_DIRECTORY'):
            self.config.test_directory = os.getenv('TEST_DIRECTORY')